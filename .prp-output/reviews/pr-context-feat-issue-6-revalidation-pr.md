...
# PR Review Context: #716 — docs: record issue #6 revalidation
...
## CRITICAL NOTE
The PR description claims only 2 files changed (.prp-output artifact files), but the actual diff contains 41 files including significant source code changes. This discrepancy must be flagged.

## Project Guidelines
- TypeScript/Bun project — MCP server for semantic memory
- Runtime: Bun (>=1.2.0), HTTP Framework: Hono, DB: SQLite + FTS5 (Drizzle ORM)
- Vector backends: Qdrant Cloud (production), LanceDB (local dev)
- Auth: OAuth 2.1 + PKCE, static Bearer token fallback
- Port: 47778
- NEVER use force flags, NEVER merge PRs without explicit user permission
...
This PR was meant to contain only 2 revalidation artifact files but actually contains 41 files with substantial source code changes including:
- OAuth 2.1 + PKCE implementation (src/oauth/*)
- Streamable HTTP MCP transport (src/mcp-transport.ts)
- TTL/expiry features (src/db/schema.ts, src/tools/search.ts, scripts/expire-learnings.ts)
- Vector backend factory refactoring (src/vector/factory.ts)
- Various configuration and infrastructure updates

## File Tier Classification
### Tier 1 (Critical — Auth/Security)
- src/oauth/provider.ts
- src/oauth/routes.ts
- src/oauth/types.ts
- src/mcp-transport.ts
...
### Tier 4 (Low Risk — Tests/Docs)
- src/tools/__tests__/search.test.ts
- src/tools/__tests__/ttl.test.ts
- src/indexer-preservation.test.ts
- src/integration/mcp-http.test.ts
- src/integration/oauth.test.ts
- .prp-output/plans/completed/*.md
- .prp-output/reports/*.md
...
+
+!AGENTS.md
diff --git a/.prp-output/plans/completed/issue-6-oracle-family-thread-normalization-revalidation.plan.md b/.prp-output/plans/completed/issue-6-oracle-family-thread-normalization-revalidation.plan.md
new file mode 100644
index 00000000..83232ec5
--- /dev/null
+++ b/.prp-output/plans/completed/issue-6-oracle-family-thread-normalization-revalidation.plan.md
@@ -0,0 +1,219 @@
+---
+status: complete
+mode: standard
...
diff --git a/.prp-output/reports/issue-6-revalidation-report-20260412-1104.md b/.prp-output/reports/issue-6-revalidation-report-20260412-1104.md
new file mode 100644
index 00000000..46deaa02
--- /dev/null
+++ b/.prp-output/reports/issue-6-revalidation-report-20260412-1104.md
@@ -0,0 +1,80 @@
...
index 8aa9f832..0d744634 100644
--- a/README.md
+++ b/README.md
@@ -7,17 +7,17 @@
...
@@ -203,6 +226,7 @@ bun test:coverage     # With coverage
 ## References
