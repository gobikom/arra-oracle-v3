...
- Runner retry on non-retryable failures (#1150): `_runner-lib.sh` retries all non-zero exits including non-recoverable ones. **Corrected scope (2026-08-22)**: exit 3 = WorkspaceError (transient), NOT gate failure as originally assumed. SCRIPT_MISSING exits as 4 (generic DAG failure). Fix needs cross-repo exit code contract in multi-agents CLI before runner can distinguish retryable from non-retryable.
- **[RESOLVED 2026-08-22] identity_mismatch spawn race (#1167, refs #1157)**: Empty `pane_current_path` during shell spawn treated as identity_mismatch, cancelling 8/15 tasks per week. Fixed in PR #1169 — retry empty cwd 4x with 3s backoff, detect pane_dead=1 with auto-respawn, distinct reason values (pane_dead, session_not_ready, identity_mismatch). Review: R1 2 Critical (hardcoded reason, missing respawn) → fixed → R3 0 issues.
...
- **[RESOLVED 2026-08-26] infra-collector false disk measurement (#1064, #1055)**: `check_disk` used `shutil.disk_usage` which diverged from `df` in mount/overlay contexts — reported 93-96% for 16 days while actual was 69%. Fixed in PR #1175: prefers `df -P -B1` with shutil fallback. Also fixed billing gate (#1062): test-mode subscriptions now WARNING not CRITICAL via exact match instead of substring.
...
- **plan-validate script** (2026-07-17, PRs #1086-#1088): `scripts/plan-validate.py` mechanically validates plan.md against gate rules YAML — 33 checks across 4 categories (pre_implement, multi_phase_epic, new_feature, impact_completeness). AND-keyword matching for YAML item mapping. Symlinked to `~/ops/bin/plan-validate`. Registered in psak.yaml tools_knowledge.
- **Kickoff Protocol** (2026-07-17, PR #1086): psak.yaml autonomous_mode kickoff_protocol — 16 mandatory items (pre-kickoff 6, execution gates 7, autonomous loop 3) + responsibility matrix (5 gates × 4 roles) + knowledge areas (7 areas × 2 roles). Generator renders as proper ### section with tables. Prevents นุด from having to repeat gate instructions at every kickoff.
- **Session-end improvement scan** (2026-07-17, PRs #1089-#1091): conductor Phase 7 HANDOFF + generator Session End §5 — interactive 5-step flow: scan → present → user confirm → execute confirmed → reflect → handoff. Applies to ALL conductor-protocol agents.
- **tmux respawn-pane SEGV fix** (2026-07-17, PR #1083): agent-runner.sh replaced `respawn-pane -k` with wait-for-pane_dead + respawn without -k. tmux 3.4 use-after-free when focusing exiting pane (fixed in 3.5, Ubuntu Noble only ships 3.4). Follow-up: agent-devops#850 (build tmux 3.5+).
